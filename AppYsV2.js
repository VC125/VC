...
// 默认解析接口
const DEFAULT_PARSE_URL = "http://nm.4688888.xyz//jiexi.php?data=6996e4d509ca23fad7897c2992ebf579&url=";

// 播放源排序和映射配置
const PLAY_SOURCE_CONFIG = {
    // 排序顺序
    order: ["极光TV", "哔哩哔哩", "芒果TV", "奇异果TV", "优酷TV"],
    
    // 播放源名称映射（将各种可能的名称映射为标准名称）
    mapping: {
        // 极光TV/腾讯相关
        "qq": "极光TV",
        "腾讯": "极光TV",
...
function normalizeSourceName(name) {
    ...
    // 检查映射表中是否有匹配的key
    for (const [key, value] of Object.entries(PLAY_SOURCE_CONFIG.mapping)) {
...
function getSourceWeight(name) {
    ...
    // 查找在排序顺序中的位置
    const index = PLAY_SOURCE_CONFIG.order.indexOf(normalizedName);
    
    // 如果在排序顺序中，返回其索引（权重越小优先级越高）
    if (index !== -1) {
        return index;
    }
    
    // 如果不在排序顺序中，返回一个大数，使其排在后面
    return 999;
}
...
function genPlayList(URL, object, json, vod, vid) {
    ...
    if (URL.includes("api.php/app")) {
        ...
        for (let i = 0; i < vodUrlWithPlayer.length; i++) {
            ...
            // 标准化播放源名称
            const normalizedFlag = normalizeSourceName(flag);
            ...
            playSources.push({
                ...
                flag: normalizedFlag,
                url: from.url,
                parse_api: from.parse_api,
                originalIndex: i
            });
            
            let purl = from.parse_api;           
            const parseUrls = parseUrlMap.get(normalizedFlag) || [];
            if (purl && !parseUrls.includes(purl)) {
                parseUrls.push(purl);
            }
            parseUrlMap.set(normalizedFlag, parseUrls); 
        }
        ...
        for (let i = 0; i < vodUrlWithPlayer.length; i++) {
            ...
            playSources.push({
                ...
                flag: normalizedFlag,
                url: from.url,
                parse_api: from.player_info.parse,
                originalIndex: i
            });
    ...
    // 对播放源进行排序
    const sortedPlaySources = sortPlaySources(playSources);
    
    // 提取排序后的播放源信息
    const playFlags = [];
    const playUrls = [];
    
    for (const source of sortedPlaySources) {
        playFlags.push(source.flag);
        playUrls.push(source.url);
    }
...
function sortPlaySources(playSources) {
    ...
    // 为每个播放源计算排序权重
    const weightedSources = playSources.map(source => {
        const weight = getSourceWeight(source.flag);
    ...
    return weightedSources.map(source => {
        // 移除临时添加的weight属性
        const { weight, ...rest } = source;
        return rest;
    });
}
...
async function play(flag, id, vipFlags) {
    try {
        // 注意：这里的flag已经是标准化后的名称（如"极光TV"）
        // 但parseUrlMap中存储的是标准化后的名称作为key
        ...
        if (!parseUrls) {
            // 尝试使用原始名称查找
            const normalizedFlag = normalizeSourceName(flag);
            if (normalizedFlag !== flag) {
                parseUrls = siteJx[normalizedFlag];
            }
            
            if (!parseUrls && siteJx.hasOwnProperty('*')) { // all jx
                parseUrls = siteJx['*'];
            } else if (!parseUrls) {
                parseUrls = [];
            }
        ...
        if (parseUrls.length === 0) {
            parseUrls = [DEFAULT_PARSE_URL];
        }

        // 确保默认解析接口始终在列表中（作为备用）
        if (!parseUrls.includes(DEFAULT_PARSE_URL)) {
            parseUrls.push(DEFAULT_PARSE_URL);
        }
