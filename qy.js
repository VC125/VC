var rule={
    ...
    timeout:5000,
    class_name:'少儿&电影&电视剧&纪录片&动漫&综艺&音乐&网络电影',
    class_url:'15&1&2&3&4&6&5&16',
    limit:20,
    play_parse:true,
    lazy:$js.toString(()=>{
        try{
            ...
            console.log(api);
            let response=fetch(api,{method:'get',headers:{'User-Agent':'okhttp/3.14.9','Content-Type':'application/x-www-form-urlencoded'}});
            let bata=JSON.parse(response);
            if(bata.url.includes("qiyi")){
                input={parse:0,url:bata.url,jx:0,danmaku:"http://127.0.0.1:9978/proxy?do=danmu&site=js&url="+input.split("?")[0]}
            }else{
                input={parse:0,url:input.split("?")[0],jx:1,danmaku:"http://127.0.0.1:9978/proxy?do=danmu&site=js&url="+input.split("?")[0]}
            }
        }catch{
            input={parse:0,url:input.split("?")[0],jx:1,danmaku:"http://127.0.0.1:9978/proxy?do=danmu&site=js&url="+input.split("?")[0]}
        }
    ...
    搜索:`js:try{let filterVodIdKeywords="游戏,原创,纪录片,片花,教育,健康,军事,脱口秀,母婴,娱乐,生活,财经,体育,科技,搞笑,音乐,旅游,时尚,资讯,微剧,漫剧";let vodIdKeywords=filterVodIdKeywords.split(',').map(k=>k.trim()).filter(k=>k!=='');let filterVodNameKeywords="采访,特辑,混剪,动态漫画,《,纪录片,片段";let vodNameKeywords=filterVodNameKeywords.split(',').map(k=>k.trim()).filter(k=>k!=='');let searchKey="";try{let urlParams=new URLSearchParams(input.split('?')[1]||'');searchKey=urlParams.get('key')||''}catch(e){let match=input.match(/key=([^&]+)/);if(match){searchKey=decodeURIComponent(match[1])}}let html=request(input);let json=JSON.parse(html);let docs=json.data.docinfos;if(vodIdKeywords.length>0||vodNameKeywords.length>0){docs=docs.filter(item=>{let info=item.albumDocInfo;let vod_id="";try{vod_id=info.channel+"$"+info.albumId}catch(e){vod_id=""}let vod_name=info.albumTitle||"";for(let keyword of vodIdKeywords){if(vod_id.includes(keyword)){return false}}for(let keyword of vodNameKeywords){if(vod_name.includes(keyword)){return false}}return true})}if(searchKey&&searchKey.trim()!==''){let keyword=searchKey.trim();docs=docs.filter(item=>{let info=item.albumDocInfo;let vod_name=info.albumTitle||"";return vod_name.toLowerCase().includes(keyword.toLowerCase())})}let d=[];docs.forEach(item=>{let info=item.albumDocInfo;d.push({title:info.albumTitle||"",desc:info.tvFocus||"",pic_url:info.albumVImage||"",url:info.channel+"$"+info.albumId})});setResult(d)}catch(e){console.log("搜索错误: "+e);setResult([])}`
}